...
Spawner::Spawner(sf::RenderWindow* _renderWindow, b2World& _world, CTextureMaster* _textureMaster, const float& _scale, float _posX, float _posY, CPlayer* _player)
{
	m_World = &_world;
...
void Spawner::Update()
{
	if (m_bSpawn && m_Slimes.size() < 30)
	{
		...
		{
			m_Slimeptr = new Slime(m_RenderWindow, *m_World, m_TextureMaster, m_Scale, 10000 , 0);
